...
#define ADDROFF_UART1	0x00006000
#define SIZE_UART1	0x00002000
...
static int cfi2_uart1_irqcontrol(struct uio_info *info, s32 irq_on)
{
	...
	if (irq_on) { 
	        #ifdef ADDR_UART1_INT_REG
	        /* enable irq */
                writew(0x0001, io_int_reg);
	        #else	
	        if (test_and_clear_bit(0, &cfi2_uart1_flags)){
			enable_irq(info->irq);
		}
		#endif
	} else { 	
...
/*
#ifdef CONFIG_OF
static struct of_device_id uio_of_cfi2_uart1_match[] = {
	{ .compatible = "dh,cfi2-uart1" },
	{ },
};

MODULE_DEVICE_TABLE(of, uio_of_cfi2_uart1_match);
module_param_string(of_id, uio_of_cfi2_uart1_match[0].compatible, 128, 0);
...
static struct platform_driver uio_cfi2_fpga_driver = {
	 ...
	 .remove = uio_cfi2_fpga_remove,
 	 .driver = {
		...
		//.of_match_table = of_match_ptr(uio_of_cfi2_uart1_match),
	},
...
//#ifndef CONFIG_OF 
    ...
    io_int_reg = ioremap((unsigned long)ADDR_UART1_INT_REG , 0x01);
    /* disable irq */
    writew(0x0000, io_int_reg);
    #endif
