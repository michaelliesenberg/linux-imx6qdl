...
static struct resource cfi2_uart0_fpga_resources[] = {
    {
        ...
        .end   = MYFPGA_BASE1 + ADDROFF_UART0 + SIZE_UART0 - 1,
        .name  = "cfi2_uart0",
...
static struct uio_info cfi2_uart0_uio_info = {
   .name = DRIVER_NAME,
   .version = "0.1",
   .irq = 0,
...
//#ifndef CONFIG_OF 
    ...
    rc = gpio_request(DHCOM_GPIOL, cfi2_uart0_uio_info.name);
    if (rc) {
        printk("Failed to request gpio: %d\n", rc);
    }

    rc = gpio_direction_input(DHCOM_GPIOL);
    if (rc) {
        printk("Failed to get gpio as input: %d\n", rc);
        gpio_free(DHCOM_GPIOL);
    }

    irq = gpio_to_irq(DHCOM_GPIOL);
    if (irq <= 0) {
...
module_init(cfi2_uart0_init_module);
module_exit(cfi2_uart0_exit_module);

MODULE_DESCRIPTION("UIO driver for the CFI2 FPGA UART0");
