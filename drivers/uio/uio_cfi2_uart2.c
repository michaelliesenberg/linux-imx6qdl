...
#define ADDROFF_UART2	0x00000C00
#define SIZE_UART2	0x00000200
...
static int cfi2_uart2_irqcontrol(struct uio_info *info, s32 irq_on)
{
	...
	if (irq_on) { 	if (test_and_clear_bit(0, &cfi2_uart2_flags)) {
			    enable_irq(info->irq);
			}
	} else { 	if (!test_and_set_bit(0, &cfi2_uart2_flags)) {
